...
teams={} # gestor -> agrega
for r in roster:
    e,cargo,gestor,meta = r['email'],r['cargo'],r['gestor'],float(r['meta'])
    ...
    if cargo=='assistente':
        mult=mult_assist(at); vlr=(at or 0)*OTE_ASSIST*mult; c_av=c_pa=c_in=0.0; base=vlr
    else:
...
for r in roster:
    ...
    else:            c_av=c_pa=c_in=0.0; mult=mult_assist(at); vlr=at*OTE_TL_NOVO*mult; base=vlr   # TL Novo (OTE)
...
for nm,peso,cm,cg in canais:
    cat=(cg/cm) if cm>0 else 0; mm=mult_coord(cat); ote=OTE_FABIO*peso; fabio_vlr+=cat*ote*mm
